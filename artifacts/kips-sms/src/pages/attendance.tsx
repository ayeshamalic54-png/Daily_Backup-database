...
function DeductionReport() {
  ...
  const fetchData = () => {
    setLoading(true);
    fetch(`/api/attendance/monthly-deductions?month=${month}&type=${type}`, { headers: authHeader() as HeadersInit })
      .then(r => r.json())
                  ...
                  {data.map((row, i) => {
                    ...
                    return (
                      <tr key={row.id} className={`border-b hover:bg-gray-50 ${hasDeduction ? "bg-red-50/30" : ""}`}>
                        ...
                        <td className="py-2.5 px-3 text-center"><span className="px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-700">{row.present}</span></td>
                        <td className="py-2.5 px-3 text-center"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${row.absent > 0 ? "bg-red-100 text-red-700" : "text-gray-400"}`}>{row.absent}</span></td>
                        <td className="py-2.5 px-3 text-center"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${row.late > 0 ? "bg-amber-100 text-amber-700" : "text-gray-400"}`}>{row.late}</span></td>
                        <td className="py-2.5 px-3 text-right font-medium text-gray-700">{(row.basicSalary ?? row.feeAmount ?? 0).toLocaleString()}</td>
                        ...
                        <td className="py-2.5 px-3 text-right font-bold text-emerald-700">{(row.netSalary ?? row.netFee ?? 0).toLocaleString()}</td>
                      </tr>
                    );
                  })}
                </tbody>
  ...
  const printPortal = createPortal(
    <div id="kips-print-portal" style={{ position: "absolute", left: "-99999px", top: "-99999px", fontFamily: "Arial, sans-serif", background: "white", color: "#111827" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <p style={{ margin: "3px 0 0", fontSize: 10, color: "#6b7280" }}>{dateLabel} — {typeFilter === "student" ? "Students" : "Staff"}</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 22 }}>
        {[
          ...
          <div key={c.label} style={{ flex: "1 1 0", border: `2px solid ${c.color}`, borderRadius: 8, padding: "12px 8px", textAlign: "center", background: "#f9fafb" }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>
          {!(attendance ?? []).length
            ? <tr><td colSpan={6} style={{ ...td, textAlign: "center", color: "#9ca3af", fontStyle: "italic" }}>No attendance records for this date</td></tr>
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg w-fit">
          ...
          <button onClick={() => setTab("deductions")}
